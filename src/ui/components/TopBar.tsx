    ...
    <header className="z-30 flex h-14 shrink-0 items-center gap-3 bg-canvas/85 px-3 backdrop-blur-xl sm:px-4">
      ...
      <div className="ml-auto flex items-center gap-1">
        ...
        */}
        ...
        {/*
          전체 절전. 새 글을 받아오는 값의 대부분은 x.com 이 자기 타임라인을 다시
          그리는 데 든다 — 덱에 가려 보이지도 않는 화면이다. 켜면 그 일을 시키지 않고,
          그동안 모든 컬럼이 멈춘다. 게임처럼 다른 일을 하는 동안 쓰라고 둔 스위치라
          설정 안이 아니라 여기 있다 — 시작 전후로 한 번씩 누르는 것이 전부다.

          컬럼 하나만 재우는 것은 그 컬럼 머리글의 번개다. 이쪽이 순간 스위치라면
          저쪽은 상시 지정이라, 여기를 껐다 켜도 저쪽 지정은 그대로 남는다.
        */}
        <button
          type="button"
          ...
          aria-label={powerSave ? '전체 절전 끄기' : '전체 절전 켜기'}
          title={
            powerSave
              ? '전체 절전 켜짐 — 모든 컬럼에 새 글이 들어오지 않습니다. 누르면 최신 글을 받아옵니다'
              : '전체 절전 — 모든 컬럼의 새 글 받아오기를 멈춥니다 (게임 등 다른 일을 할 때). 컬럼 하나만 멈추려면 그 컬럼 머리글의 번개를 누르세요'
          }
        >
