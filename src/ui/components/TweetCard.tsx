...
function LinkCard({ card, mediaSize }: { card: NonNullable<Tweet['card']>; mediaSize: MediaSize }) {
  ...
  // 가리켰을 때의 테두리도 인용 상자와 같은 경계선 색이다 — 버튼 색을 쓰면 라이트
  // 테마에서 검은 선이 그어져 상자 하나가 유난히 도드라진다.
  const shell = `mt-2.5 overflow-hidden rounded-xl border border-line bg-surface-2 transition-colors group-hover/card:bg-surface-3 ${
    card.url ? 'hover:border-line-strong' : ''
  }`
...
function TranslateBlock({ tweet, textClass }: { tweet: Tweet; textClass: string }) {
  ...
  const label = busy ? '번역하는 중…' : text ? (shown ? '번역 숨기기' : '번역 보기') : '번역하기'
  ...
  return (
    <div className="mt-1">
      ...
      <button
        ...
        onClick={(event) => {
          event.stopPropagation()
          if (text) setShown(!shown)
          else void run()
        }}
        className={`text-[13px] font-medium text-accent hover:underline ${busy ? 'opacity-60' : ''}`}
      >
        {label}
      </button>

      {error && (
        <p className="mt-1 text-[12px] text-danger" role="status">
          번역하지 못했습니다 — {error}
        </p>
      )}
    </div>
    ...
    >
      ...
      <div className={`flex ${metrics.gap}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          {translatable(tweet) && <TranslateBlock tweet={tweet} textClass={metrics.text} />}
