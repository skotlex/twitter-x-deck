...
/**
 * 같은 영상을 몇 번까지 세워보고 포기할지.
 *
 * 세 번이면 상대가 물러설 뜻이 없다는 것을 알기에 충분하다. 낮게 잡을수록 싸움이
 * 짧게 끝나지만, 한 번 튕겨나간 재생을 정상적으로 다시 세우지 못하고 놔주게 된다.
 */
export const STOP_LIMIT = 3

export interface StopLedger {
  /** 이 영상을 지금 세워도 되는가. 세려는 횟수를 함께 센다. */
...
/**
 * 세운 횟수를 영상마다 세어 두고 한도를 넘으면 포기한다.
 *
 * 세우면 x.com 플레이어가 다시 트는 자리가 있었다. 그대로 두면 초당 수백 번을
