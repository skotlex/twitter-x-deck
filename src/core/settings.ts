...
export interface Settings {
   ...
   * 절전. 켜면 새 글을 **받아오지 않는다.**
   *
   * 새 글을 받아오는 일의 값은 대부분 우리 것이 아니다 — 두드리면 x.com 이 응답을
   * 주면서 **자기 타임라인도 함께 다시 그린다.** 우리는 그 화면을 보여주지 않으므로
   * 통째로 낭비인데, 그리지 말라고 할 방법이 없다. 그래서 아예 두드리지 않는다.
   *
   * **컬럼은 그동안 멈춰 있다.** 처음에는 알약만 읽어 몇 건이 밀렸는지 보여줄 수
   * 있으리라 봤지만 그렇지 않았다 — x.com 은 우리가 두드려야 새 글을 가져오고,
   * 가만 두면 알약도 뜨지 않는다. 그래서 건수를 약속하지 않고 머리글에 '절전' 이라고
   * 적어 둔다. 멈춘 것과 고장난 것은 화면에서 구별되어야 한다.
   *
   * 끄면 그 자리에서 최신 글을 받아온다. 새로고침을 눌러도 마찬가지다 — 절전이
   * 막으려는 것은 저절로 도는 일이지 사용자의 조작이 아니다.
   *
