...
describe('countUniqueSince', () => {
   ...
   * 옛 항목을 만나도 접지 않는다.
   *
   * 예전에는 목록이 늘 관측 시각 내림차순이라 첫 옛 항목에서 끊었다. 알림 컬럼은
   * 글 자체의 시각 순서로 그려지므로 방금 관측한 항목이 목록 한가운데 앉는다 —
   * 앞에서 끊으면 세다 만다.
   */
  it('옛 항목 뒤에 섞인 새 항목도 센다', () => {
    expect(countUniqueSince([slice(300, 500, 100, 400)])).toBe(2)
  })
