...
띄울 컬럼은 설정에서 자유롭게 켜고 끌 수 있으며, 컬럼 머리글을 끌어다 놓아 순서를 바꿀 수 있습니다. 머리글을 누르면 그 컬럼이 맨 위로 올라갑니다. 알림 컬럼에는 게시물과 알림(좋아요 · 팔로우 · 리포스트)이 섞여 시간순으로 쌓입니다.
...
**표시** — 띄울 컬럼, 자동 진입 여부, 테마, 글꼴, 카드 밀도, 컬럼 테두리, 카드 구분선, 미디어 표시 방식 · 자동 재생 · 크기
...
| 브리지 연결 | `npm run bridge` 가 찍어준 열쇠와 포트(기본 8765)를 넣습니다 |
...
브리지 열쇠만은 예외입니다. 그 사본은 x.com 페이지의 저장소에 놓이는데 그곳은 x.com 에서 도는 어떤 스크립트든 읽을 수 있어서, 열쇠는 사본을 남기지 않고 `chrome.storage.local` 에만 둡니다. 기기를 옮기면 다시 넣어야 합니다.
...
- 띄우는 컬럼이 많을수록 x.com 화면을 그만큼 더 열어두게 되므로 메모리 사용량과 갱신 지연이 늘어납니다
