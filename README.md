...
이쪽만 방식이 다릅니다. Papago 이미지 번역은 네이버 로그인이 필요한데, 로그인 쿠키는 Papago 가 **최상위인 문서** 에만 실립니다(`SameSite=Lax`). 그래서 보이지 않는 프레임으로는 할 수 없고, 배경 워커가 **최소화한 창** 을 따로 띄워 처리한 뒤 결과 사진만 가져와 그 창을 닫습니다. 보던 창의 탭 목록은 건드리지 않습니다. 완전히 안 보이게 할 수는 없습니다 — 크롬이 주는 최상위 문서는 탭과 창뿐입니다.

로그인 여부는 탭을 열기 전에 **세션 쿠키가 있는지** 로 확인합니다. 화면을 뒤져 알아내는 방법도 있지만 번번이 어긋났습니다 — 로그인 안내가 아예 안 뜨는 경우도 있고, 로그인 상태의 사용자 메뉴가 숨은 채로 문서에 늘 들어 있기도 합니다. 로그인이 안 돼 있으면 탭을 열지 않고 곧바로 안내하며, 로그인할지는 사용자가 정합니다 — 사진을 보던 중에 다른 탭으로 끌려가지 않습니다. 로그인 화면에서 돌아온 탭은 스스로 닫힙니다.
...
| `cookies` | 사진 번역 전에 네이버 세션 쿠키가 있는지만 확인합니다. 없으면 번역 탭을 열지 않고 로그인 안내를 띄웁니다. 확인 대상은 papago.naver.com 하나이며 값은 쓰지 않습니다 |
