...
그래도 응답이 들어오지 않으면 강제 갱신 사다리를 차례로 오릅니다 — 알림 클릭 → 홈 링크 재클릭 → 탭 재클릭 → `.` 단축키 → 문서 새로고침. 응답이 들어오면 다시 맨 아래로 내려옵니다.
...
- x.com 의 화면 구조나 응답 형식이 바뀌면 수집이 멈출 수 있습니다. 컬럼 머리글의 상태 배지(`준비 중` · `수신 중` · `폴백 파싱` · `교대 수집`)가 진단 지점입니다
