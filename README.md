...
x.com 의 **추천 · 팔로잉 · 알림 · 멘션** 타임라인을 한 화면에 나란히 놓고 실시간으로 받아보는 크롬 확장입니다.
...
| 알림 | `x.com/notifications` |
| 멘션 | `x.com/notifications/mentions` |

각 타임라인을 컬럼으로 띄울지, 종으로만 지켜볼지, 아예 끌지는 설정에서 고르며, 컬럼 머리글을 끌어다 놓아 순서를 바꿀 수 있습니다. 머리글을 누르면 그 컬럼이 맨 위로 올라갑니다. 알림 컬럼에는 게시물과 알림(좋아요 · 팔로우 · 리포스트)이 섞여 시간순으로 쌓입니다.
...
안 본 수는 덱을 연 뒤에 들어온 글만 셉니다. 보관해둔 과거 글은 세지 않습니다.
...
- **사진 번역** — 사진 속 일본어 · 영어를 한국어로 옮깁니다 (선택 기능, 아래 참고)

### 사진 속 글자 번역 (선택)

라이트박스에서 사진 하나를 골라 그 안의 글자를 한국어로 옮깁니다. 세로쓰기 일본어도 대상입니다.

이 기능만은 브라우저 안에서 끝나지 않습니다. 이 PC 에 깔린 `codex` · `claude` 명령과 그 **구독 계정** 을 그대로 빌려 쓰며, 둘 사이를 잇는 작은 로컬 서버([bridge/](bridge/README.md))를 함께 띄워야 합니다. 별도 API 키나 추가 요금은 필요하지 않습니다.

| 고른 명령 | 결과 |
| --- | --- |
| Codex | 글자를 한국어로 바꿔 **다시 그린 사진**. 원본과 번역본을 오갈 수 있습니다 |
...
| `npm run bridge` | 사진 번역 브리지를 띄웁니다 (선택 기능을 쓸 때만) |
...
**타임라인** 은 네 종류마다 **끔 · 컬럼 · 종** 중 하나를 고릅니다. 이 자리에서는 알림을 **알림(전체)** 로 적습니다 — 바로 아래 멘션과 나란히 놓이는데, 알림은 멘션까지 포함한 전체 목록이기 때문입니다. 컬럼은 화면에 자리를 차지하고, 종은 자리를 쓰지 않고 상단 바의 종에 안 본 수만 띄웁니다. 창이 좁아 컬럼을 늘릴 수 없을 때 종으로 돌리면 배치를 지킨 채 확인할 수 있습니다. 컬럼은 최소 하나 남겨야 하므로 마지막 컬럼은 다른 쪽으로 옮길 수 없습니다.
...
| 브리지 | `npm run bridge` 로 띄워두기만 하면 알아서 찾아옵니다. 붙여넣거나 맞출 값이 없습니다 |
...
'새 게시물 보기' 알림이 떠 있으면 눌러 다음 타임라인을 끌어옵니다. 그것으로도 컬럼이 조용하면 강제 갱신 사다리를 차례로 오릅니다 — 내비 링크 재클릭 → 탭 재클릭 → `.` 단축키 → 프레임 재적재. 내비 링크는 담당 화면의 것을 씁니다(추천 · 팔로잉은 홈 링크, 멘션 · 알림은 알림 링크). 팔로잉만 순서가 다릅니다 — 홈 링크로 받아오는 것은 홈의 기본 탭인 추천이라 그 칸을 맨 뒤로 미룹니다.

판정 기준은 하나뿐입니다 — **그 컬럼의 응답이 들어왔는가.** 알림을 눌렀다거나 탭을 눌렀다는 '시도' 는 근거로 치지 않습니다. 옆 컬럼의 응답, 목록과 무관한 응답도 마찬가지입니다(멘션 · 알림 프레임은 응답 이름을 가릴 수 없어 그 화면의 응답을 전부 받으므로, 목록처럼 생긴 것만 골라 셉니다). 응답이 들어오면 사다리는 맨 아래로 내려오고, 그러지 않으면 20초마다 다음 칸으로 오릅니다. 같은 이유로 컬럼 머리글의 `수신 중` 배지도 그 컬럼이 실제로 응답을 받은 뒤에야 켜집니다.
...
글 번역과 달리 이쪽은 브라우저 밖의 도움을 받습니다. 확장은 파일도 프로세스도 만질 수 없는데, 쓰려는 것은 이 PC 에 깔린 `codex` · `claude` 명령이기 때문입니다. 그 사이를 [bridge/server.mjs](bridge/server.mjs) 가 잇습니다 — 의존성 없는 작은 Node 서버입니다.

```
라이트박스 '사진 번역'
└─ 배경 워커            ← x.com 문서는 로컬 서버를 직접 부르지 못합니다
   └─ 127.0.0.1 의 8765 부터 훑어 브리지를 찾습니다
      ├─ 이미지를 내려받아 임시 파일로 둡니다
      ├─ codex exec -i <파일> …  또는  claude -p …
      └─ 결과를 덱으로 돌려주고 임시 파일을 지웁니다
```

브리지는 8765 부터 비어 있는 포트를 골라 앉고, 확장은 같은 범위를 훑어 찾아옵니다. 그 자리에 남의 서버가 있을 수 있으므로 우리라는 표시를 확인한 뒤에야 쓰며, 찾은 번호는 기억해 두었다가 다음부터는 곧바로 부릅니다. 브리지를 껐다 켜 다른 번호로 옮겨 앉아도 다시 훑어 따라갑니다.
...
bridge/          사진 번역용 로컬 서버 (선택 기능. 확장 번들에 들어가지 않습니다)

tests/
├─ core/          파서 · 설정 · 세션 힌트 테스트
├─ content/       선택자 판단 규칙 테스트 (손으로 만든 DOM)
├─ ui/            표시 형식 · 안 본 수 세기 · 계정 읽기 테스트
...
| [bridge/server.mjs](bridge/server.mjs) | `codex` · `claude` 를 대신 부르는 로컬 서버. 로그인 판정도 여기서 합니다 |
...
| `host_permissions: http://127.0.0.1/*` | 사진 번역 브리지를 부르는 데 씁니다. x.com 문서는 로컬 서버로 연결하지 못해 배경 워커가 대신 나갑니다. 이 PC 안으로만 나가며, 사진 번역을 켜지 않으면 한 번도 쓰이지 않습니다 |
...
- `127.0.0.1` 에만 묶습니다. 같은 망의 다른 기기는 들어올 수 없습니다
- 요청에 `x-deck-bridge` 헤더가 있어야 합니다. 웹페이지가 이 헤더를 붙이려는 순간 브라우저가 먼저 예비 요청(preflight)을 보내는데 브리지는 그것을 허락하지 않으므로, 본 요청 자체가 나가지 못합니다. 확장은 호스트 권한이 있어 그 검사를 거치지 않습니다
...
**주고받는 열쇠는 없습니다.** 한때 넣었다가 걷어냈습니다 — 열쇠 파일은 브리지 옆에 평문으로 놓이므로, 이 PC 에서 도는 악성 프로그램이라면 그것을 먼저 읽으면 그만입니다. 막아야 할 것(다른 웹사이트)은 위의 조건들로 막히고, 열쇠는 사용자에게 붙여넣기라는 단계만 하나 더 얹고 있었습니다.

브리지를 띄우지 않으면 이 경로는 존재하지 않습니다.
...
- 사진 번역은 브리지를 띄워 둔 동안에만 쓸 수 있습니다. Codex 로 사진을 다시 그리는 데는 한 장에 30초 ~ 1분 남짓 걸리므로, 눌러두고 나중에 확인하는 쪽에 가깝습니다
