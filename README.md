...
판정 기준은 하나뿐입니다 — **그 컬럼의 응답이 들어왔는가.** 알림을 눌렀다거나 탭을 눌렀다는 '시도' 는 근거로 치지 않습니다. 옆 컬럼의 응답도 마찬가지입니다. 응답이 들어오면 사다리는 맨 아래로 내려오고, 그러지 않으면 20초마다 다음 칸으로 오릅니다. 같은 이유로 컬럼 머리글의 `수신 중` 배지도 그 컬럼이 실제로 응답을 받은 뒤에야 켜집니다.
