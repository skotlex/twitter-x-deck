...
| 번개 아이콘 | **절전.** 새 글을 받아오지 않고 몇 건이 밀렸는지만 세어 둡니다. 켜져 있으면 단추에 색이 들어옵니다 |
...
새 글 한 뭉치를 받아오는 값의 대부분은 덱이 아니라 x.com 쪽에서 듭니다. '새 게시물 보기' 알림을 누르면 x.com 은 응답을 주면서 **자기 타임라인도 함께 다시 그립니다.** 덱에 가려 보이지도 않는 화면인데 React 재실행부터 스타일 재계산 · 레이아웃까지 전부 치릅니다.

상단 바의 번개 단추를 켜면 그 알림을 누르지 않습니다. 누르지 않아도 알림은 계속 뜨므로 **몇 건이 밀려 있는지는 컬럼 머리글에 그대로 보입니다.** 끄는 순간 밀린 것을 한 번에 받아옵니다 — 놓치는 것이 아니라 미뤄두는 것입니다.
